...
flatten = @(A)A(:);
% FIXME keepWhen is a hack for flattenStruct bug where signal updates
% before all fields have a value
...
switch lower(newelem.grating)
  case {'sinusoid' 'sine' 'sin'}
    [gratingLayer, gratingImg] = vis.tex.sinusoidLayer(newelem.azimuth,...
      newelem.spatialFreq, newelem.phase, newelem.orientation);
    gratingLayer.textureId = 'sinusoidGrating';
  case {'squarewave' 'square' 'sq'}
    [gratingLayer, gratingImg] = vis.tex.squareWaveLayer(newelem.azimuth,...
      newelem.spatialFreq, newelem.phase, newelem.orientation);
...
% 'rgbaSize' field
[gratingLayer.rgba, gratingLayer.rgbaSize] = vis.tex.rgba(gratingImg, 1);
...
if ~strcmpi(newelem.window, 'none')
  switch lower(newelem.window)
    case {'gaussian' 'gauss'}
      [winLayer, winImg] = vis.tex.gaussianLayer(...
  ...
  [winLayer.rgba, winLayer.rgbaSize] = vis.tex.rgba(0, winImg);
