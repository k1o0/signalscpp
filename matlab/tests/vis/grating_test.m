...
assert(strcmp(elem.Node.Value.grating, 'sinusoid'), 'Unexpected grating type default')
assert(strcmp(elem.Node.Value.window, 'gaussian'), 'Unexpected window default')
assert(all(structfun(@isempty, elem.Node.Value) == false), 'Undefined default parameters')
...
assert(isequal(elem.Node.Value.colour.Node.Value, [0 0 0]))
assert(isequal(elem.Node.Value.sigma.Node.Value, [20 20]))
% all vectors in 'layers' struct should be column vectors
assert(isequal(elem.Node.Value.layers.Node.Value.maxColour, [0 0 0 1]'))
...
assert(isequal(elem.Node.Value.sigma.Node.Value, [20 20]))
% all vectors in 'layers' struct should be column vectors
assert(isequal(elem.Node.Value.layers.Node.Value(2).maxColour, [0 0 0 1]'))
